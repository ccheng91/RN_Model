...
data.photo <- dplyr::filter(data.photo, data.photo$species != "bat" & data.photo$species != "commongreenmagpie" & 
                              ...
                              data.photo$species!="treeshrew"& data.photo$species != "junglefowl"& data.photo$species!="silverpheasant"& 
                              data.photo$species!="squirrel"& data.photo$species!="bird" & 
                              data.photo$species!="rat" & data.photo$species != "unknown" & data.photo$species != "human" & data.photo$species != "human2"
                            & data.photo$species != "hunter"& data.photo$species != "dog"& data.photo$species != "cattle"& data.photo$species != "watermonitor")

#table(data.photo$species)
#write.csv(data.photo, file="data/All_mammal_photo.csv", row.names = FALSE)
######################################################################
...
q <- which(data.photo$species == "bat" | data.photo$species == "commongreenmagpie" | 
             ...
             data.photo$species=="squirrel"| data.photo$species=="bird" | 
             data.photo$species=="rat" | data.photo$species == "unknown" | data.photo$species == "human")
##### how many camera station? ####
NOstie <- table(data.photo$camera, data.photo$species)
a <- which(NOstie > 0)
NOstie[a] <- 1
NOsite <- colSums(NOstie)
subset(NOsite, NOstie != 0 )
NOstie <-NOsite[which(NOsite != 0)]
write.csv(NOsite,file="data/sitecount.csv" ) 
...
p <- ggplot(list2, aes(x=spp, y = count, order=count)) 
