...
pa <- c("Bulangshan", "Mango", "Mengla", "Menglun", "Mengsong", "Nabanhe")

## 
reach <- aggregate(qs$Time_of_outreach ~ qs$PAs, FUN = "mean")
reach.sd <- aggregate(qs$Time_of_outreach ~ qs$PAs, FUN = "sd")
punish <- aggregate(qs$No_of_Punishment ~ qs$PAs, FUN = "mean")
punish.sd <- aggregate(qs$No_of_Punishment ~ qs$PAs, FUN = "sd")
...
qsn <- as.matrix(table(qs$PAs))
reach.sd[,2] <- reach.sd[,2]/qsn
punish.sd[,2] <- punish.sd[,2]/qsn
...
# new data frame
anthrop <- data.frame(pa, reach[,2], punish[,2], relationship[,2], Score.P[,2], Score.H[,2], wtedu[,2],income[,2], reach.sd[,2],punish.sd[,2])
names(anthrop) <- c("pas","reach","punish","relationship", "score.P","score.H", "edu", "income","reach.sd","punish.sd")
...
anthrop$size <- size$size * 10000

## Linear model
z <- lm(richness ~ size, data=anthrop)
P1 <- ggplot(anthrop, aes(y=richness, x=reach) )+ geom_point() +theme_classic()+ theme(axis.title  = element_text(size = 20),axis.text = element_text(size = 15))+
  #geom_errorbar(aes(ymax = RAI.wildboar + RAI.SD.wildboar, ymin = RAI.wildboar - RAI.SD.wildboar,width = .05))+
  geom_errorbarh(aes(xmax = reach + reach.sd, xmin = reach - reach.sd, height = .1)) + xlim(2,7) +
  theme(axis.line.x = element_line(), axis.line.y = element_line()) + geom_text(aes(label=pas),hjust=0, vjust=-0.25, color="Blue",size = 7,check_overlap = TRUE) +
  labs(x="Freq of villager-reported outreach (times/year)",y="Number of species detected", size=5) + geom_smooth(method='lm',color="black",linetype=2, se=FALSE) 

P2 <- ggplot(anthrop, aes(y=richness, x=punish) )+ geom_point() +theme_classic()+ theme(axis.title  = element_text(size = 20),axis.text = element_text(size = 15))+
  #geom_errorbar(aes(ymax = RAI.wildboar + RAI.SD.wildboar, ymin = RAI.wildboar - RAI.SD.wildboar,width = .05))+
  geom_errorbarh(aes(xmax = punish + punish.sd, xmin = punish - punish.sd, height = .1))+xlim(0.4,1.4) +
  theme(axis.line.x = element_line(), axis.line.y = element_line()) + geom_text(aes(label=pas),hjust=0, vjust=-0.25, color="Blue",size = 7,check_overlap = TRUE) + 
  labs(x="Freq of villager-reported punishment (times/year)",y="Number of species detected", size=5) + geom_smooth(method='lm',color="black", linetype=2, se=FALSE) 

P3 <- ggplot(anthrop, aes(y=richness, x=size) )+ geom_point() +theme_classic()+ theme(axis.title  = element_text(size = 20),axis.text = element_text(size = 15))+
   xlim(0,100000) +#geom_errorbar(aes(ymax = RAI.wildboar + RAI.SD.wildboar, ymin = RAI.wildboar - RAI.SD.wildboar,width = .05))+
  #geom_errorbarh(aes(xmax = Reach + Reach.SD, xmin = Reach - Reach.SD, height = .1))+
  theme(axis.line.x = element_line(), axis.line.y = element_line()) + geom_text(aes(label=pas),hjust=0, vjust=-0.25, color="Blue",size = 7,check_overlap = TRUE)+  
  labs(x="Size of Protected area (Ha)",y="Number of species detected", size=5) + geom_smooth(method='lm',color="black",linetype=2, se=FALSE) 
...
z3 <-lm(richness ~ punish, data=anthrop)
summary(z)
summary(z2)
summary(z3)
